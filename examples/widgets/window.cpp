...
Window::Window(MainWindow *mainWindow)
    ...
    styles.emplace_back("https://maps.mapvina.com/styles/v2/streets.json?key=public_key", "Demo Tiles");

    QMapVina::Settings settings;
    settings.setStyles(styles);
    settings.setDefaultZoom(5);
    settings.setDefaultCoordinate(QMapVina::Coordinate(43, 21));

    m_mapWidget = std::make_unique<QMapVina::MapWidget>(settings).release();
    ...
    setWindowTitle(tr("Hello QMapVina"));
}
