...
/*!
    \defgroup QMapVinaWidgets QMapVina Widgets
    \brief Qt Widgets for MapVina.
*/

/*!
    \class MapWidget
    \brief A QRhiWidget-based MapVina map widget with cross-platform rendering support.
    \ingroup QMapVinaWidgets

    \headerfile map_widget.hpp <QMapVinaWidgets/MapWidget>
    ...
    It owns its own instance of \ref QMapVina::Map that is rendered to the widget.
...
/*!
    \brief Get the QMapVina::Map instance.
*/
Map *MapWidget::map() {
...
#ifdef MLN_RENDERER_DEBUGGING
                    qDebug() << "MapWidget::initialize() - Using Qt's Vulkan device for MapVina renderer";
...
void MapWidget::render(QRhiCommandBuffer *cb) {
    ...
    if (glTextureId != 0) {
        // Pass the OpenGL texture to MapVina for zero-copy rendering
    ...
    if (metalTexturePtr != nullptr) {
        // Pass the Metal texture to MapVina for zero-copy rendering
        // We need to set this every frame as the texture might change
