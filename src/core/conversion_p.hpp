...
public:
...
#endif
               || QString(value.typeName()) == QStringLiteral("QMapVina::Feature") ||
               value.userType() == qMetaTypeId<QVector<QMapVina::Feature>>() ||
               value.userType() == qMetaTypeId<QList<QMapVina::Feature>>() ||
               value.userType() == qMetaTypeId<std::list<QMapVina::Feature>>();
    }
    ...
    static std::optional<GeoJSON> toGeoJSON(const QVariant &value, Error &error) {
        if (value.typeName() == QStringLiteral("QMapVina::Feature")) {
            return GeoJSON{QMapVina::GeoJSON::asFeature(value.value<QMapVina::Feature>())};
        }

        if (value.userType() == qMetaTypeId<QVector<QMapVina::Feature>>()) {
            return featureCollectionToGeoJSON(value.value<QVector<QMapVina::Feature>>());
        }

        if (value.userType() == qMetaTypeId<QList<QMapVina::Feature>>()) {
            return featureCollectionToGeoJSON(value.value<QList<QMapVina::Feature>>());
        }

        if (value.userType() == qMetaTypeId<std::list<QMapVina::Feature>>()) {
            return featureCollectionToGeoJSON(value.value<std::list<QMapVina::Feature>>());
        }
...
private:
    ...
    static GeoJSON featureCollectionToGeoJSON(const T &features) {
        ...
        for (const auto &feature : features) {
            collection.push_back(QMapVina::GeoJSON::asFeature(feature));
        }
