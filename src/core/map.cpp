...
static_assert(mbgl::underlying_type(QMapVina::Map::NorthUpwards) ==
                  mbgl::underlying_type(mbgl::NorthOrientation::Upwards),
              "error");
static_assert(mbgl::underlying_type(QMapVina::Map::NorthRightwards) ==
                  mbgl::underlying_type(mbgl::NorthOrientation::Rightwards),
              "error");
static_assert(mbgl::underlying_type(QMapVina::Map::NorthDownwards) ==
                  mbgl::underlying_type(mbgl::NorthOrientation::Downwards),
              "error");
static_assert(mbgl::underlying_type(QMapVina::Map::NorthLeftwards) ==
...
mbgl::MapOptions mapOptionsFromSettings(const QMapVina::Settings &settings, const QSize &size, qreal pixelRatio) {
...
mbgl::ResourceOptions resourceOptionsFromSettings(const QMapVina::Settings &settings) {
...
mbgl::ClientOptions clientOptionsFromSettings(const QMapVina::Settings &settings) {
    return std::move(mbgl::ClientOptions()
                         .withName(settings.clientName().toStdString())
                         .withVersion(settings.clientVersion().toStdString()));
}

std::optional<mbgl::Annotation> asAnnotation(const QMapVina::Annotation &annotation) {
    auto asGeometry = [](const QMapVina::ShapeAnnotationGeometry &geometry) {
        mbgl::ShapeAnnotationGeometry result;
        switch (geometry.type) {
            case QMapVina::ShapeAnnotationGeometry::LineStringType:
                result = QMapVina::GeoJSON::asLineString(geometry.geometry.first().first());
                break;
            case QMapVina::ShapeAnnotationGeometry::PolygonType:
                result = QMapVina::GeoJSON::asPolygon(geometry.geometry.first());
                break;
            case QMapVina::ShapeAnnotationGeometry::MultiLineStringType:
                result = QMapVina::GeoJSON::asMultiLineString(geometry.geometry.first());
                break;
            case QMapVina::ShapeAnnotationGeometry::MultiPolygonType:
                result = QMapVina::GeoJSON::asMultiPolygon(geometry.geometry);
                break;
        }
        return result;
    };

    if (annotation.canConvert<QMapVina::SymbolAnnotation>()) {
        const auto symbolAnnotation = annotation.value<QMapVina::SymbolAnnotation>();
        const QMapVina::Coordinate &pair = symbolAnnotation.geometry;
        return {
            mbgl::SymbolAnnotation(mbgl::Point<double>{pair.second, pair.first}, symbolAnnotation.icon.toStdString())};
    }

    if (annotation.canConvert<QMapVina::LineAnnotation>()) {
        const auto lineAnnotation = annotation.value<QMapVina::LineAnnotation>();
    ...
    if (annotation.canConvert<QMapVina::FillAnnotation>()) {
        const auto fillAnnotation = annotation.value<QMapVina::FillAnnotation>();
...
/*!
    \class Map
    \brief The Map class is a Qt wrapper for the MapVina Native engine.
    \ingroup QMapVina

    \headerfile map.hpp <QMapVina/Map>

    Map is a Qt friendly version the MapVina Native engine using Qt types
...
/*!
    ...
    be any as defined by the <a href="https://mapvina.io/github/mapvina-style-spec/">
    MapVina Style Spec</a> for layout properties.
    ...
    This table describes the mapping between <a href="https://mapvina.io/github/mapvina-style-spec/#types">
    style types</a> and Qt types accepted by setLayoutProperty():

    MapVina style type  |  Qt type
    ---------------------|---------------------
    Enum                 | \c QString
    String               | \c QString
    Boolean              | \c bool
...
/*!
    ...
    Sets a layout property value to an existing layer. The property string can
    be any as defined by the <a href="https://mapvina.io/github/mapvina-style-spec/">
    MapVina Style Spec</a> for paint properties.
...
/*!
    ...
    <a href="https://mapvina.io/github/mapvina-style-spec/#root-sources">MapVina Style Specification</a>
    with parameters.
...
/*!
    ...
    <a href="https://mapvina.io/github/mapvina-style-spec/#root-layers">MapVina style specification</a>
...
/*!
    ...
    <a href="https://mapvina.io/github/mapvina-style-spec/deprecations/#other-filter">MapVina Style Spec</a>.
...
/*!
    ...
    <a href="https://mapvina.io/github/mapvina-style-spec/types/">MapVina Style Spec</a>.
*/
