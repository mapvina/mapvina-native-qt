...
// MapVina Native supports geometry segments that spans above 180 degrees in
...
QMapVina::Coordinates qgeocoordinate2mapboxcoordinate(const QList<QGeoCoordinate> &crds,
                                                       const bool crossesDateline,
                                                       bool closed = false) {
    QMapVina::Coordinates coordinates;
    for (const QGeoCoordinate &coordinate : crds) {
        if (!coordinates.empty() && crossesDateline &&
            ...
            coordinates << QMapVina::Coordinate{
                coordinate.latitude(),
                coordinate.longitude() + (coordinate.longitude() >= 0 ? -fullCircle : fullCircle)};
        } else {
            coordinates << QMapVina::Coordinate{coordinate.latitude(), coordinate.longitude()};
        }
    }
...
namespace QMapVina::StyleChangeUtils {
