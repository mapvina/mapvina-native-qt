...
Item {
    ...
    Plugin {
        ...
        PluginParameter {
            ...
            value: [
                {
                    "url": "https://maps.mapvina.com/styles/v2/streets.json?key=public_key",
                    "name": "Demo Tiles",
                    "description": "MapVina Demo Tiles",
                    "type": MapType.PedestrianMap
                },
                {
                    "url": "https://maps.mapvina.com/styles/v2/streets.json?key=public_key",
                    "name": "Demo Tiles Night",
                    "description": "MapVina Demo Tiles for night usage",
                    "night": true,
    ...
    TestCase {
        ...
        function test_plugin_style_values() {
            ...
            compare(mapView.map.supportedMapTypes[0].description, "MapVina Demo Tiles")
            compare(mapView.map.supportedMapTypes[0].night, false)
            compare(mapView.map.supportedMapTypes[0].style, MapType.PedestrianMap)
            compare(mapView.map.supportedMapTypes[0].metadata["url"], "https://maps.mapvina.com/styles/v2/streets.json?key=public_key")

            compare(mapView.map.supportedMapTypes[1].name, "Demo Tiles Night")
            compare(mapView.map.supportedMapTypes[1].description, "MapVina Demo Tiles for night usage")
            ...
            compare(mapView.map.supportedMapTypes[1].metadata["url"], "https://maps.mapvina.com/styles/v2/streets.json?key=public_key")
