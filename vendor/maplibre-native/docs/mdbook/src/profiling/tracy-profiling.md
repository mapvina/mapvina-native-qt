...
MapVina Native integrates [Tracy profiler](https://github.com/wolfpld/tracy) which offers an easy way to understand and optimize your application's CPU and GPU performance.
...
Instrumentation is generally the first step in profiling applications that use MapVina. Once slow inner-loop routines are identified, additional hardware vendor specific tools can be used to collect hardware counters and optimize low level CPU and GPU code.
...
#### Enabling instrumentation in MapVina Native
...
OpenGL is also supported in MapVina native. Tracy support is currently missing for other APIs such as Metal and need to be added separately.
...
The above macros can be added inside MapVina code and also in the application code that calls MapVina.

The following macros should only be used if there are changes to MapVina internals:
...
Make sure you generate the MapVina project with the option `MLN_USE_TRACY` enabled.

As an example, the glfw sample is used.

With CMake, in MapVina repository root do
~~~bash
# generate project
...
#### Connecting the profiler to a MapVina Android application
