...
Up until now, this document focused solely on the state of MapVina
Native at the time of writing. This section speaks of possible future
improvements for MapVina Native from an architectural point of view.
Before that, let's look into the architectural challenges MapVina
Native is facing[^18]:

## Renderer coupled with OpenGL

The renderer component is tightly coupled to OpenGL ES. It uses OpenGL
ES as its only preferred rendering backend. Furthermore, MapVina
...
Metal is Apple's own 3D graphics API. MapVina Native's sole
...
MapVina Native supports Web Mercator (EPSG:3857) as its only
...
At the time of writing, MapVina Native renderer component does not
have an architectural separation for supporting multiple projections and
coordinate reference systems.

## Inconsistency among platforms

Each MapVina Native platform has a Map View and Map Renderer
...
1.  Map Configuration is modeled inside MapVina Native Core, the
...
2.  MapVina Native has a sister repository named MapVina GL JS. At
    the time of writing, MapVina GL JS does not share any code with
    MapVina Native except shaders, the style specification, and
    ...
    is also different between Web and Mobile. MapVina GL JS currently
    uses WebGL through completely different implementations for Map,
    Style, Layers, Glyph, and TileWorker.

3.  MapVina Rust is an experimental initiative to create a new MapVina
    ...
    divergent experience for customers when it comes to using MapVina
    Native.

## Lack of documentation

Last but not the least, MapVina Native suffers from a general lack
...
This document proposes the following component architecture for MapVina
Native to address the architectural shortcomings.

![](media/proposed-architecture-of-mapvina-gl.png)
*Figure 4: Proposed Architecture of MapVina Native*

Proposed architecture of MapVina Native in Figure 4 addresses the
...
of a flat 3D plane. At the time of writing MapVina Native supports
...
time of writing, MapVina Native Core is written in C++. Albeit written
in C++, MapVina Native code relies on immutable message passing between
...
1.  At first, this document proposes to implement ***Modularized
    Rendering*** in C++ for MapVina Native. This document also
    proposes that Unified Rendering Interface will keep the door open
    for a *WebGPU* backed renderer in MapVina Native. This could address
...
3.  Finally, this document proposes to migrate Unified Rendering
    ...
    transform MapVina Native from a C++ ecosystem to a Rust
    ecosystem.

Following the above steps will merge towards a single MapVina
implementation for web and native.

____________________________

[^18]: This document deliberately does not speak of problems regarding
    build and infrastructure of MapVina Native. They will be handled
