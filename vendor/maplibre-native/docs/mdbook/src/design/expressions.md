...
2.  Expressions apply rendering transformations to filtered features.
    MapVina offers expressions that can interpolate, and paint. An
...
**Although it looks like JavaScript, for MapVina Native**, **the
parser for any expression is written in MapVina Native Core**. Each
...
up in the MapVina Native Core.

Also, inside MapVina Native Core, this definition mechanism allows
...
**To render the built expression, MapVina Native uses expression
parsers.** **Expression parsers are written in MapVina Native Core
