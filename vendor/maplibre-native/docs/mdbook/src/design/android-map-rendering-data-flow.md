...
Before the map initialization request makes it to MapVina Native
...
like other components inside MapVina works as a state machine. The
...
*MapVina Native* renderer. This overall transform directive, such as
...
rendering, MapVina Native only renders *dirty* tiles. A dirty tile
is a tile rendered in the viewport that has changed due to user
interaction. To initiate this process, MapVina Native loads the
tileset to be rendered first. In a rendering request, if the tileset is
already loaded, MapVina Native will use a cached tile set.

The next decision to make here is which tiles are to be rendered on the
viewport. To deduce this, MapVina Native translates the device
