...
*Figure 1: MapVina Native Components – Ten Thousand Foot view*

From ten thousand foot, MapVina Native is composed of *Map View* and a
*Renderer*. MapVina Native supports iOS, Android, Linux, QT, MacOS,
...
Apart from the platform code, *MapVina Native* offers shared
...
Style component embodies a *MapVina Style Document*. A style document
...
*Layer* is an overloaded terminology in the MapVina Native's
...
MapVina Native is used in mobile platforms. To be performant in
underpowered environments, MapVina Native tries to leverage message
...
MapVina Native uses a *Renderer* component to translate map tiles
fetched from a tile server to a rendered map.

MapVina Native uses a pool of *workers*. These workers are
...
concurrency/threading primitives for MapVina Native core to use. The
...
We have noted early in this document that MapVina Native uses message
...
Following that same trend, MapVina Native offers tile worker for
...
are. MapVina Native offers a base Tile class. Raster, Geometry, and
Elevation tile instances are inherited from Tile base class. Through
this inheritance MapVina Native maintains the association of Tile
...
[^6]: In the Javascript counterpart, MapVina GL JS, this is achieved
