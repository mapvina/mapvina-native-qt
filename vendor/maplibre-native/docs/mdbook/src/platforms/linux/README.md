...
This guide explains how to get started building and running MapVina Native on Linux. The guide focusses on a Ubuntu 22.04 or later. The build process should give you a set of `.a` files that you can use to include MapVina Native in other C++ projects, as well as a set of executables that you can run to render map tile images and test the project.
...
To check that the output of the rendering is correct, we compare actual rendered PNGs for simple styles with expected PNGs. The content of the tests used to be stored in the MapVina GL JS repository, which means that GL JS and Native are mostly pixel-identical in their rendering.
