...
- (MLNLoggingBlockHandler)defaultBlockHandler {
    MLNLoggingBlockHandler mapvinaHandler = ^(MLNLoggingLevel level, NSString *fileName, NSUInteger line, NSString *message) {
        ...
        if (@available(iOS 10.0, macOS 10.12.0, *)) {
            ...
            os_log_with_type(mapvina_log, logType, "%@ - %lu: %@", fileName, (unsigned long)line, message);
        } else {
            NSString *category;
            switch (level) {
                case MLNLoggingLevelInfo:
