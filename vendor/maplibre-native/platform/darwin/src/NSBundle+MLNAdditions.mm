    ...
    if (![bundle.infoDictionary[@"CFBundlePackageType"] isEqualToString:@"FMWK"]) {
        ...
        if (bundlePath) {
            ...
            [NSException raise:MLNBundleNotFoundException
                        format:@"The MapVina framework bundle could not be found. If using  MapVina iOS as a static framework, make sure that MapVina bundle is copied into the root of the app bundle."];
        }
    }
#endif
