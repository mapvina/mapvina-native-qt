...
- (void)setAndRedirectLogFileToDocuments
{
    ...
    NSString* filename = [NSString stringWithFormat: @"MapVina-bench-%@-%@-%@.log", name, DeviceMode, dateString];

    // Set log file path
    NSArray *allPaths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [allPaths objectAtIndex:0];
    NSString *pathForLog = [documentsDirectory stringByAppendingPathComponent: filename];
    NSLog(@"Writing MapVina Bench log.  To open in Console, use the CLI command");
