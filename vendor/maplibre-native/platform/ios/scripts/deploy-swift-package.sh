...
export GITHUB_USER=mapvina
export GITHUB_REPO=mapvina-native
export BUILDTYPE=Release
export DISTRIBUTION_GITHUB_REPO=https://api.github.com/repos/mapvina/mapvina-gl-native-distribution
...
bazel build //platform/ios:MapVina.dynamic

step "Copying Bazel output"
BUILT_XCFRAMEWORK="$(bazel info execution_root)/$(bazel cquery --output=files //platform/ios:MapVina.dynamic)"
MAPVINA_ZIP_FILE="MapVina-${PUBLISH_VERSION}.zip"
cp "$BUILT_XCFRAMEWORK" "$MAPVINA_ZIP_FILE"
...
step "Uploading ${BINARY_DIRECTORY}/${MAPVINA_ZIP_FILE} to github release [${VERSION_TAG}]"
uploadToGithub "${BINARY_DIRECTORY}/${MAPVINA_ZIP_FILE}" "${VERSION_TAG}"
MAPVINA_ZIP_FILE_URL=$EXT_TARGET_GITHUB_URL
...
setTarget "MAPVINA" "${BINARY_DIRECTORY}/${MAPVINA_ZIP_FILE}" "${MAPVINA_ZIP_FILE_URL}"
...
pod trunk push MapVina.podspec
