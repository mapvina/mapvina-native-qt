...
def define_env(env):
    ...
    def activity_source_note(filename):
        file_path = find_file(f"{os.getcwd()}/MapVinaAndroidTestApp", filename)

        return f"""
!!! note

    You can find the full source code of this example in [`{filename}`](https://github.com/mapvina/mapvina-native/blob/main/platform/android/{file_path}) of the MapVinaAndroidTestApp.
"""
