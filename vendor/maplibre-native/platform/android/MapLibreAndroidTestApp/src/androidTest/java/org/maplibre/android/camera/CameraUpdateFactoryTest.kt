...
class CameraUpdateFactoryTest : BaseTest() {
    ...
    fun testLatLngBoundsUntiltedUnrotated() {
        mapvinaMap.cameraPosition = CameraPosition.Builder()
    ...
    fun testLatLngBoundsWithProvidedTiltAndRotation() {
        ...
        mapvinaMap.moveCamera(CameraUpdateFactory.newLatLngBounds(bounds, 30.0, 40.0, 0))
    ...
    fun withPadding_cameraInvalidated_paddingPersisting() {
        val initialCameraPosition = mapvinaMap.cameraPosition
        val initialPoint = mapvinaMap.projection.toScreenLocation(initialCameraPosition.target!!)
        ...
        mapvinaMap.moveCamera(CameraUpdateFactory.paddingTo(leftPadding.toDouble(), 0.0, 0.0, bottomPadding.toDouble()))

        Assert.assertArrayEquals(intArrayOf(leftPadding, 0, 0, bottomPadding), mapvinaMap.padding)

        val resultingCameraPosition = mapvinaMap.cameraPosition
        ...
        assertEquals(
            ...
            mapvinaMap.projection.toScreenLocation(resultingCameraPosition.target!!)
        )
    ...
    fun withLatLngPadding_cameraInvalidated_paddingPersisting() {
        ...
        mapvinaMap.moveCamera(CameraUpdateFactory.newLatLngPadding(expectedTarget, 0.0, topPadding.toDouble(), rightPadding.toDouble(), 0.0))

        Assert.assertArrayEquals(intArrayOf(0, topPadding, rightPadding, 0), mapvinaMap.padding)
        ...
        val centerLatLng = mapvinaMap.projection.fromScreenLocation(PointF((mapView.width / 2).toFloat(), (mapView.height / 2).toFloat()))
