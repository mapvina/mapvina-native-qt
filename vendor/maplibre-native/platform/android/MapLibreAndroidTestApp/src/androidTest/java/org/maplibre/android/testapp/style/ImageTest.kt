...
class ImageTest : EspressoTest() {
    ...
    fun testAddGetImage() {
        ...
        MapVinaMapAction.invoke(mapvinaMap) { uiController, mapvinaMap ->
            ...
            mapvinaMap.style!!.addImage(IMAGE_ID, bitmapSet)

            // adding an image requires converting the image with an asynctask
            uiController.loopMainThreadForAtLeast(200)

            val bitmapGet = mapvinaMap.style!!.getImage(IMAGE_ID)
            assertTrue(bitmapGet!!.similarTo(bitmapSet))

            mapvinaMap.style!!.removeImage(IMAGE_ID)
            assertNull(mapvinaMap.style!!.getImage(IMAGE_ID))
        }
    }
}
