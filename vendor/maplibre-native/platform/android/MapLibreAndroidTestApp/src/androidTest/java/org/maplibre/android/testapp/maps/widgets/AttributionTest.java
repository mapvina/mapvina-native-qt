...
public class AttributionTest extends EspressoTest {
  ...
  public void testMapboxStreetsMapboxAttributionLink() {
    ...
    onView(withText(R.string.mapvina_attributionsDialogTitle)).check(matches(isDisplayed()));
  ...
  private void buildUrlSpans() {
    onView(withId(R.id.mapView)).perform(new MapVinaMapAction((uiController, view) -> {
      for (Source source : mapvinaMap.getStyle().getSources()) {
  ...
  private class DisableAction implements ViewAction {
    ...
    public void perform(UiController uiController, View view) {
      mapvinaMap.getUiSettings().setAttributionEnabled(false);
    }
  }

  private class MapVinaMapAction implements ViewAction {

    private InvokeViewAction invokeViewAction;

    MapVinaMapAction(InvokeViewAction invokeViewAction) {
