...
public abstract class BaseTest extends AppCenter {
  ...
  @UiThread
  @CallSuper
  protected void initMap(MapVinaMap mapvinaMap) {
    this.mapvinaMap = mapvinaMap;
    mapvinaMap.getStyle(style -> latch.countDown());
  }

  protected void validateTestSetup() {
    if (!MapVina.isConnected()) {
      Timber.e("Not connected to the internet while running test");
    }
    assertNotNull("MapView isn't initialized", mapView);
    assertNotNull("MapVinaMap isn't initialized", mapvinaMap);
    assertNotNull("Style isn't initialized", mapvinaMap.getStyle());
    assertTrue("Style isn't fully loaded", mapvinaMap.getStyle().isFullyLoaded());
  }
