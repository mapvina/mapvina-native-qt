...
import com.mapvina.android.testapp.databinding.ActivityRecyclerviewBinding
    ...
    class ItemAdapter(private val context: Context, private val inflater: LayoutInflater, private val fragmentManager: androidx.fragment.app.FragmentManager) : androidx.recyclerview.widget.RecyclerView.Adapter<androidx.recyclerview.widget.RecyclerView.ViewHolder>() {
        ...
        class MapPagerAdapter(private val context: Context, fm: FragmentManager) : FragmentStatePagerAdapter(fm, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT) {
            ...
            override fun getItem(position: Int): androidx.fragment.app.Fragment {
                val options = MapVinaMapOptions.createFromAttributes(context)
                ...
                when (position) {
                    0 -> {
                        ...
                        fragment.getMapAsync { mapvinaMap -> mapvinaMap.setStyle(TestStyles.getPredefinedStyleWithFallback("Streets")) }
                        return fragment
                    }
                    1 -> {
                    ...
                    2 -> {
                        ...
                        val fragment = SupportMapFragment.newInstance(options)
                        fragment.getMapAsync { mapvinaMap -> mapvinaMap.setStyle(TestStyles.getPredefinedStyleWithFallback("Pastel")) }
                        return fragment
                    }
                    3 -> {
                        return EmptyFragment.newInstance()
                    ...
                    4 -> {
                        ...
                        val fragment = SupportMapFragment.newInstance(options)
                        fragment.getMapAsync { mapvinaMap -> mapvinaMap.setStyle(TestStyles.getPredefinedStyleWithFallback("Satellite Hybrid")) }
                        return fragment
                    }
                    5 -> {
                        return EmptyFragment.newInstance()
