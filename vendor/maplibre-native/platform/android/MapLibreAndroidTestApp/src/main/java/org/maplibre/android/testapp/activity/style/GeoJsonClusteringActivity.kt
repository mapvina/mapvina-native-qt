...
class GeoJsonClusteringActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView.getMapAsync {
            ...
            mapvinaMap.addOnMapClickListener { latLng: LatLng? ->
                val point = mapvinaMap.projection.toScreenLocation(latLng!!)
                val features =
                    mapvinaMap.queryRenderedFeatures(point, "cluster-0", "cluster-1", "cluster-2")
                if (!features.isEmpty()) {
    ...
    private fun onClusterClick(cluster: Feature, clickPoint: Point) {
        if (clickOptionCounter == 0) {
            ...
            val zoomDelta = nextZoomLevel - mapvinaMap.cameraPosition.zoom
            mapvinaMap.animateCamera(
