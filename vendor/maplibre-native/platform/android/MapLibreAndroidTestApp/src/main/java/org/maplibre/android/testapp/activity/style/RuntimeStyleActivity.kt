...
class RuntimeStyleActivity : AppCompatActivity() {
    ...
    private fun setLayerInvisible() {
        ...
        for (roadLayer in roadLayers) {
            val layer = mapvinaMap.style!!.getLayer(roadLayer)
    ...
    private fun setBackgroundOpacity() {
        val background = mapvinaMap.style!!.getLayer("background")
        background?.setProperties(PropertyFactory.backgroundOpacity(0.2f))
    }

    private fun setWaterColor() {
        val water = mapvinaMap.style!!.getLayerAs<FillLayer>("water")
        if (water != null) {
    ...
    private fun removeBuildings() {
        ...
        mapvinaMap.style!!.removeLayer("building")
    }
    ...
    private fun addParksLayer() {
        ...
        mapvinaMap.style!!.addLayerBelow(layer, "building")
        ...
        mapvinaMap.style!!.getLayer("parksLayer")!!
        ...
        layer = mapvinaMap.style!!.getLayerAs("parksLayer")
        ...
        mapvinaMap.animateCamera(CameraUpdateFactory.zoomTo(12.0))
    }
    ...
    private fun addDynamicParksLayer() {
        ...
        // Add an empty source
        mapvinaMap.style!!.addSource(GeoJsonSource("dynamic-park-source"))
        ...
        mapvinaMap.style!!.addLayer(layer)
        ...
        handler.postDelayed(
            {
                ...
                val source = mapvinaMap.style!!.getSourceAs<GeoJsonSource>("dynamic-park-source")
                if (source == null) {
    ...
    private fun addTerrainLayer() {
        ...
        if (latestLayer != null) {
            mapvinaMap.style!!.addLayerBelow(layer, latestLayer.id)
        }

        // Need to get a fresh handle
        layer = mapvinaMap.style!!.getLayerAs("terrainLayer")

        // Make sure it's also applied after the fact
        layer!!.minZoom = 10f
        layer.maxZoom = 15f
        layer = mapvinaMap.style!!.getLayer("terrainLayer") as LineLayer?
        Toast.makeText(
            this,
            String.format(
    ...
    private fun updateWaterColorOnZoom() {
        val layer = mapvinaMap.style!!.getLayerAs<FillLayer>("water") ?: return
        ...
        mapvinaMap.animateCamera(CameraUpdateFactory.zoomTo(1.0), 1500)
    }
    ...
    private fun addCustomTileSource() {
        ...
        mapvinaMap.style!!.addLayer(lineLayer)
    }

    private fun styleFillColorLayer() {
        mapvinaMap.setStyle(Style.Builder().fromUri("asset://fill_color_style.json"))
    ...
    private fun styleFillFilterLayer() {
        mapvinaMap.setStyle(Style.Builder().fromUri("asset://fill_filter_style.json"))
        mapvinaMap.moveCamera(
            CameraUpdateFactory.newLatLngZoom(
                LatLng(31.0, (-100).toDouble()),
                3.0
        ...
        handler.postDelayed(
            {
                ...
                val states = mapvinaMap.style!!.getLayer("states") as FillLayer?
                if (states != null) {
        ...
        handler.postDelayed(
            {
                ...
                val states = mapvinaMap.style!!.getLayer("state-label-lg") as SymbolLayer?
                if (states != null) {
                    states.setProperties(
    ...
    private fun styleLineFilterLayer() {
        mapvinaMap.setStyle(Style.Builder().fromUri("asset://line_filter_style.json"))
        mapvinaMap.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(40.0, (-97).toDouble()), 5.0))
        val handler = Handler(mainLooper)
        handler.postDelayed(
            {
                if (!this::mapvinaMap.isInitialized) {
                ...
                val counties = mapvinaMap.style!!.getLayer("counties") as LineLayer?
                if (counties != null) {
    ...
    private fun styleNumericFillLayer() {
        mapvinaMap.setStyle(Style.Builder().fromUri("asset://numeric_filter_style.json"))
        mapvinaMap.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(40.0, (-97).toDouble()), 5.0))
        val handler = Handler(mainLooper)
        handler.postDelayed(
            {
                ...
                val regions = mapvinaMap.style!!.getLayer("regions") as FillLayer?
                if (regions != null) {
                    regions.setFilter(
                        Expression.all(
                            Expression.gte(
    ...
    private fun bringWaterToFront() {
        val water = mapvinaMap.style!!.getLayer("water")
        if (water != null) {
            mapvinaMap.style!!.removeLayer(water)
            mapvinaMap.style!!.addLayerAt(water, mapvinaMap.style!!.layers.size - 1)
        } else {
