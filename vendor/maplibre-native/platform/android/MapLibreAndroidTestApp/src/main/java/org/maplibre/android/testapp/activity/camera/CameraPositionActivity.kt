...
import com.mapvina.android.constants.GeometryConstants
...
class CameraPositionActivity : FragmentActivity(), OnMapReadyCallback, View.OnClickListener, OnMapLongClickListener {
    ...
    override fun onClick(view: View) {
        ...
        builder.setPositiveButton("Animate", DialogClickListener(mapvinaMap, dialogContent))
    ...
    private fun toggleLogCameraChanges() {
        ...
        if (logCameraChanges) {
            mapvinaMap.addOnCameraIdleListener(idleListener)
            mapvinaMap.addOnCameraMoveCancelListener(moveCanceledListener)
            // # --8<-- [start:addOnCameraMoveListener]
            mapvinaMap.addOnCameraMoveListener(moveListener)
            // # --8<-- [end:addOnCameraMoveListener]
            mapvinaMap.addOnCameraMoveStartedListener(moveStartedListener)
        } else {
            mapvinaMap.removeOnCameraIdleListener(idleListener)
            mapvinaMap.removeOnCameraMoveCancelListener(moveCanceledListener)
            mapvinaMap.removeOnCameraMoveListener(moveListener)
            mapvinaMap.removeOnCameraMoveStartedListener(moveStartedListener)
        }
    }
    ...
    override fun onDestroy() {
        ...
        if (::mapvinaMap.isInitialized) {
            mapvinaMap.removeOnMapLongClickListener(this)
        }
    ...
    private class DialogClickListener(private val mapvinaMap: MapVinaMap?, private val dialogContent: View) : DialogInterface.OnClickListener {
