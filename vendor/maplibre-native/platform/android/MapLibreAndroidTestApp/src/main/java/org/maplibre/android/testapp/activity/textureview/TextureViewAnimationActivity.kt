...
class TextureViewAnimationActivity : AppCompatActivity() {
    ...
    private fun setupMapView(savedInstanceState: Bundle?) {
        ...
        mapView.getMapAsync { mapvinaMap: MapVinaMap ->
            this@TextureViewAnimationActivity.mapvinaMap = mapvinaMap
            mapvinaMap.setStyle(TestStyles.getPredefinedStyleWithFallback("Streets"))
            setFpsView(mapvinaMap)
            ...
            flyTo(mapvinaMap, 0, 14.0)
        }
    }

    private fun flyTo(mapvinaMap: MapVinaMap, place: Int, zoom: Double) {
        mapvinaMap.animateCamera(
            ...
            object : CancelableCallback {
                ...
                override fun onFinish() {
                    flyTo(mapvinaMap, if (place == PLACES.size - 1) 0 else place + 1, zoom)
                }
            }
        )
    }

    private fun setFpsView(mapvinaMap: MapVinaMap) {
        val fpsView = findViewById<View>(R.id.fpsView) as TextView
        mapvinaMap.setOnFpsChangedListener { fps: Double ->
