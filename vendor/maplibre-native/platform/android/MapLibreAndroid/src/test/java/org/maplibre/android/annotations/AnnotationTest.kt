...
class AnnotationTest {
    ...
    fun testRemove() {
        annotation!!.id = 1
        annotation!!.setMapVinaMap(mapvinaMap)
        annotation!!.remove()
        Mockito.verify(mapvinaMap, Mockito.times(1)).removeAnnotation(
            annotation!!
        )
    }

    @Test
    fun testRemoveUnboundMapVinaMap() {
        annotation!!.id = 1
        annotation!!.remove()
        Mockito.verify(mapvinaMap, Mockito.times(0)).removeAnnotation(
            annotation!!
        )
    }
