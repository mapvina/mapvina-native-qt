...
import com.mapvina.android.location.LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS
...
class LocationComponentTest {
    ...
    private lateinit var developerAnimationListeners: List<MapVinaMap.OnDeveloperAnimationListener>
    ...
    fun before() {
        ...
        locationComponent = LocationComponent(mapvinaMap, transform, developerAnimationListeners, currentListener, lastListener, locationLayerController, locationCameraController, locationAnimatorCoordinator, staleStateManager, compassEngine, false)
        doReturn(style).`when`(mapvinaMap).style
    ...
    fun transitionCallbackFinishedTest() {
        ...
        locationComponent.onStart()
        locationComponent.isLocationComponentEnabled = true
        `when`(mapvinaMap.cameraPosition).thenReturn(CameraPosition.DEFAULT)
    ...
    fun change_to_gps_mode_symbolLayerBearingValue() {
        ...
        `when`(mapvinaMap.projection).thenReturn(projection)
        `when`(mapvinaMap.cameraPosition).thenReturn(CameraPosition.DEFAULT)
    ...
    fun tiltWhileTracking_notReady() {
        ...
        val callback = mock(MapVinaMap.CancelableCallback::class.java)
    ...
    fun newLocation_accuracy_indicatorLayerRadiusValue() {
        val location = Location("test")
        location.accuracy = 50f
        locationComponent = LocationComponent(mapvinaMap, transform, developerAnimationListeners, currentListener, lastListener, locationLayerController, locationCameraController, locationAnimatorCoordinator, staleStateManager, compassEngine, true)
        locationComponent.activateLocationComponent(
            LocationComponentActivationOptions.builder(context, style)
                .locationComponentOptions(locationComponentOptions)
