...
class TransformTest {
    ...
    @Test
    fun testMoveCamera() {
        val mapvinaMap = mockk<MapVinaMap>()
        every { mapvinaMap.cameraPosition } answers { CameraPosition.DEFAULT }
        ...
        transform.moveCamera(mapvinaMap, update, callback)
    ...
    fun testMoveCameraToSamePosition() {
        ...
        transform.moveCamera(mapvinaMap, update, null) // Initialize camera position
        transform.moveCamera(mapvinaMap, update, callback)
    ...
    fun testEaseCamera() {
        ...
        transform.easeCamera(mapvinaMap, update, 100, false, callback)
    ...
    fun testEaseCameraToSamePosition() {
        ...
        val expected = CameraPosition.DEFAULT
        val update = CameraUpdateFactory.newCameraPosition(expected)
        transform.moveCamera(mapvinaMap, update, null)
    ...
    fun testAnimateCamera() {
        ...
        transform.animateCamera(mapvinaMap, update, 100, callback)
    ...
    fun testCancelNotInvokedFromOnFinish() {
        ...
        val callback = object : MapVinaMap.CancelableCallback {
            override fun onCancel() {
                throw IllegalStateException("onCancel shouldn't be called from onFinish")
            }

            override fun onFinish() {
                transform.animateCamera(mapvinaMap, CameraUpdateFactory.newCameraPosition(expected), 500, null)
            }
        }
        transform.animateCamera(mapvinaMap, CameraUpdateFactory.newCameraPosition(expected), 500, callback)
    }
}
