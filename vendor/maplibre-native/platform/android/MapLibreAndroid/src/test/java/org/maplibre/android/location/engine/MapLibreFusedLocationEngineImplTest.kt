...
class MapVinaFusedLocationEngineImplTest {
    @Mock
    private val locationManagerMock: LocationManager? = null
    private var engine: LocationEngine? = null
    private var mapvinaFusedLocationEngineImpl: MapVinaFusedLocationEngineImpl? = null
    @Before
    fun setUp() {
        val context = mock(Context::class.java)
        `when`(context.getSystemService(Context.LOCATION_SERVICE)).thenReturn(locationManagerMock)
        mapvinaFusedLocationEngineImpl = MapVinaFusedLocationEngineImpl(context)
        engine = LocationEngineProxy(mapvinaFusedLocationEngineImpl)
    }
    ...
    fun createListener() {
        ...
        val locationListener = mapvinaFusedLocationEngineImpl!!.createListener(callback)
...
// J2k: // TODO: see https://github.com/mapvina/mapvina-native/issues/1949
/*
