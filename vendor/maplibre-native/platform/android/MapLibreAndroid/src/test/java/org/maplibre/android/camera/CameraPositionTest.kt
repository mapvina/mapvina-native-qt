...
import com.mapvina.android.camera.CameraUpdateFactory.zoomTo
...
class CameraPositionTest {
    ...
    fun testTypedArrayBuilder() {
        ...
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.mapvina_MapView_mapvina_cameraBearing,
                0.0f
            )
        ).thenReturn(bearing)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.mapvina_MapView_mapvina_cameraTargetLat,
                0.0f
            )
        ).thenReturn(latitude)
        Mockito.`when`(
            typedArray.getFloat(
                R.styleable.mapvina_MapView_mapvina_cameraTargetLng,
                0.0f
            )
        ).thenReturn(longitude)
        Mockito.`when`(typedArray.getFloat(R.styleable.mapvina_MapView_mapvina_cameraZoom, 0.0f))
            .thenReturn(zoom)
        Mockito.`when`(typedArray.getFloat(R.styleable.mapvina_MapView_mapvina_cameraTilt, 0.0f))
            .thenReturn(tilt)
