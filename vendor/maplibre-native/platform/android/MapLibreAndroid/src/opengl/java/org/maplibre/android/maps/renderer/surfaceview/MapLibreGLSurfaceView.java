...
import com.mapvina.android.maps.renderer.egl.EGLLogWrapper;
...
public class MapVinaGLSurfaceView extends MapVinaSurfaceView {

  protected final WeakReference<MapVinaGLSurfaceView> viewWeakReference = new WeakReference<>(this);
  ...
  public MapVinaGLSurfaceView(Context context) {
    super(context);
  }

  public MapVinaGLSurfaceView(Context context, AttributeSet attrs) {
    super(context, attrs);
  }
  ...
  private static class EglHelper {
    private EglHelper(WeakReference<MapVinaGLSurfaceView> glSurfaceViewWeakRef) {
    ...
    public void start() {
      try {
        ...
        MapVinaGLSurfaceView view = mGLSurfaceViewWeakRef.get();
        if (view == null) {
          mEglConfig = null;
          mEglContext = null;
        } else {
    ...
    private WeakReference<MapVinaGLSurfaceView> mGLSurfaceViewWeakRef;
    EGL10 mEgl;
   ...
   */
  static class GLThread extends MapVinaSurfaceView.RenderThread {
    GLThread(WeakReference<MapVinaGLSurfaceView> surfaceViewWeakRef) {
    ...
    protected void guardedRun() throws InterruptedException {
      ...
      try {
        ...
        while (true) {
          synchronized (renderThreadManager) {
            while (true) {
              ...
              if (pausing && haveEglContext) {
                MapVinaGLSurfaceView view = mSurfaceViewWeakRef.get();
          ...
          if (createEglContext) {
            MapVinaSurfaceView view = mSurfaceViewWeakRef.get();
            if (view != null) {
     ...
     */
    protected WeakReference<MapVinaGLSurfaceView> mSurfaceViewWeakRef;
  }
}
