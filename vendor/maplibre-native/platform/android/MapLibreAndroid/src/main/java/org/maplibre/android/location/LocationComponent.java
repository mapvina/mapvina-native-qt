...
import com.mapvina.android.location.engine.MapVinaFusedLocationEngineImpl;
...
import com.mapvina.android.maps.MapVinaMap.OnCameraMoveListener;
...
import static com.mapvina.android.location.LocationComponentConstants.DEFAULT_FASTEST_INTERVAL_MILLIS;
import static com.mapvina.android.location.LocationComponentConstants.DEFAULT_INTERVAL_MILLIS;
import static com.mapvina.android.location.LocationComponentConstants.DEFAULT_TRACKING_PADDING_ANIM_DURATION;
import static com.mapvina.android.location.LocationComponentConstants.DEFAULT_TRACKING_TILT_ANIM_DURATION;
import static com.mapvina.android.location.LocationComponentConstants.DEFAULT_TRACKING_ZOOM_ANIM_DURATION;
import static com.mapvina.android.location.LocationComponentConstants.TRANSITION_ANIMATION_DURATION_MS;
import static com.mapvina.android.location.modes.RenderMode.GPS;
 ...
 * <p>
 * <strong>
 * To get the component object use {@link MapVinaMap#getLocationComponent()} and activate it with
 ...
 * {@link MapVinaFusedLocationEngineImpl}.
...
public final class LocationComponent {
  ...
  /**
   * Indicates that the component is enabled and should be displaying location if MapVina components are available and
   ...
   * This allows MapVina components enter started state and display data, and adds state safety for methods like
   * {@link #setLocationComponentEnabled(boolean)}
   */
  private boolean isComponentStarted;

  /**
   * Indicates if MapVina components are ready to be interacted with. This can differ from {@link #isComponentStarted}
   * if the MapVina style is being reloaded.
   */
   ...
   * Internal use.
   * <p>
   * To get the component object use {@link MapVinaMap#getLocationComponent()}.
   */
  public LocationComponent(@NonNull MapVinaMap mapvinaMap,
                           @NonNull Transform transform,
                           @NonNull List<MapVinaMap.OnDeveloperAnimationListener> developerAnimationListeners) {
  ...
  @VisibleForTesting
  LocationComponent(@NonNull MapVinaMap mapvinaMap,
                    @NonNull Transform transform,
                    @NonNull List<MapVinaMap.OnDeveloperAnimationListener> developerAnimationListeners,
  ...
  public void activateLocationComponent(@NonNull LocationComponentActivationOptions activationOptions) {
    ...
    if (options == null) {
      ...
      if (styleRes == 0) {
        styleRes = R.style.mapvina_LocationComponent;
      }
  ...
  private class CameraTransitionListener implements OnLocationCameraTransitionListener {
    ...
    private void reset(@CameraMode.Mode int cameraMode) {
      locationAnimatorCoordinator.resetAllCameraAnimations(mapvinaMap.getCameraPosition(),
   ...
   * use one of {@link MapVinaMap#moveCamera(CameraUpdate)},
   * {@link MapVinaMap#easeCamera(CameraUpdate)} or {@link MapVinaMap#animateCamera(CameraUpdate)} instead.
   * <p>
  ...
  public void zoomWhileTracking(double zoomLevel, long animationDuration,
    ...
    locationAnimatorCoordinator.feedNewZoomLevel(
      zoomLevel, mapvinaMap.getCameraPosition(), animationDuration, callback
    );
  }
  ...
  /**
   * Cancels animation started by {@link #zoomWhileTracking(double, long, MapVinaMap.CancelableCallback)}.
   */
  ...
  public void paddingWhileTracking(double[] padding, long animationDuration,
    ...
    locationAnimatorCoordinator.feedNewPadding(padding, mapvinaMap.getCameraPosition(), animationDuration, callback);
  }

  /**
   * Cancels animation started by {@link #paddingWhileTracking(double[], long, MapVinaMap.CancelableCallback)}.
   */
  ...
  public void tiltWhileTracking(double tilt, long animationDuration,
    ...
    locationAnimatorCoordinator.feedNewTilt(tilt, mapvinaMap.getCameraPosition(), animationDuration, callback);
  }
  ...
  /**
   * Cancels animation started by {@link #tiltWhileTracking(double, long, MapVinaMap.CancelableCallback)}.
   */
   ...
   * <p>
   * Setting this <b>will not impact</b> any other animations schedule with {@link MapVinaMap}, gesture animations or
   ...
   * <pre>
   * {@code
   * MapVinaMap.addOnCameraIdleListener(new MapVinaMap.OnCameraIdleListener() {
   *   {@literal @}Override
   ...
   * {@link MapVinaMap.OnMapClickListener#onMapClick(LatLng)} is going to be consumed
   ...
   * {@link MapVinaMap.OnMapLongClickListener#onMapLongClick(LatLng)} is going to be consumed
   * and not pushed to the listeners registered after the component's activation.
   *
   * @param listener The location click listener that is invoked when the
   *                 location is clicked
  ...
  private void onLocationLayerStart() {
    if (!isComponentInitialized || !isComponentStarted || mapvinaMap.getStyle() == null) {
      return;
    }
    ...
    if (!isLayerReady) {
      ...
      mapvinaMap.addOnCameraIdleListener(onCameraIdleListener);
  ...
  private void onLocationLayerStop() {
    ...
    mapvinaMap.removeOnCameraMoveListener(onCameraMoveListener);
    mapvinaMap.removeOnCameraIdleListener(onCameraIdleListener);
  }
  ...
  private void initialize(@NonNull final Context context, @NonNull Style style, boolean useSpecializedLocationLayer,
    ...
    mapvinaMap.addOnMapClickListener(onMapClickListener);
    mapvinaMap.addOnMapLongClickListener(onMapLongClickListener);
    ...
    locationLayerController = new LocationLayerController(mapvinaMap, style, sourceProvider, featureProvider,
    ...
    locationCameraController = new LocationCameraController(
      context, mapvinaMap, transform, cameraTrackingChangedListener, options, onCameraMoveInvalidateListener);

    locationAnimatorCoordinator = new LocationAnimatorCoordinator(
      mapvinaMap.getProjection(),
      MapVinaAnimatorSetProvider.getInstance(),
      MapVinaAnimatorProvider.getInstance()
    );
  ...
  private void updateLocation(@Nullable final Location location, @Nullable List<Location> intermediatePoints,
    ...
    CameraPosition currentCameraPosition = mapvinaMap.getCameraPosition();
  ...
  private void updateCompassHeading(float heading) {
    locationAnimatorCoordinator.feedNewCompassBearing(heading, mapvinaMap.getCameraPosition());
  }
  ...
  private void updateLayerOffsets(boolean forceUpdate) {
    ...
    CameraPosition position = mapvinaMap.getCameraPosition();
  ...
  private void updateAccuracyRadius(Location location, boolean noAnimation) {
    ...
    if (location == null) {
      ...
      radius = Utils.calculateZoomLevelRadius(mapvinaMap, location);
    }
  ...
  @NonNull
  private MapVinaMap.OnMapLongClickListener onMapLongClickListener = new MapVinaMap.OnMapLongClickListener() {
    @Override
  ...
  @NonNull
  private final MapVinaMap.OnDeveloperAnimationListener developerAnimationListener =
    new MapVinaMap.OnDeveloperAnimationListener() {
      @Override
  ...
  private void notifyUnsuccessfulCameraOperation(@Nullable MapVinaMap.CancelableCallback callback,
                                                 @Nullable String msg) {
    if (msg != null) {
      Logger.e(TAG, msg);
    }
