...
public class InfoWindow {
  ...
  private WeakReference<MapVinaMap> mapvinaMap;
  ...
  InfoWindow(MapView mapView, int layoutResId, MapVinaMap mapvinaMap) {
    layoutRes = layoutResId;
    View view = LayoutInflater.from(mapView.getContext()).inflate(layoutResId, mapView, false);
    initialize(view, mapvinaMap);
  }

  InfoWindow(@NonNull View view, MapVinaMap mapvinaMap) {
    initialize(view, mapvinaMap);
  }

  private void initialize(@NonNull View view, MapVinaMap mapvinaMap) {
    this.mapvinaMap = new WeakReference<>(mapvinaMap);
    isVisible = false;
    ...
    view.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        MapVinaMap mapvinaMap = InfoWindow.this.mapvinaMap.get();
        if (mapvinaMap != null) {
          MapVinaMap.OnInfoWindowClickListener onInfoWindowClickListener = mapvinaMap.getOnInfoWindowClickListener();
    ...
    view.setOnLongClickListener(new View.OnLongClickListener() {
      ...
      public boolean onLongClick(View v) {
        MapVinaMap mapvinaMap = InfoWindow.this.mapvinaMap.get();
        if (mapvinaMap != null) {
          MapVinaMap.OnInfoWindowLongClickListener listener = mapvinaMap.getOnInfoWindowLongClickListener();
          if (listener != null) {
  ...
  private void closeInfoWindow() {
    MapVinaMap mapbox = mapvinaMap.get();
  ...
  InfoWindow open(@NonNull MapView mapView, Marker boundMarker, @NonNull LatLng position, int offsetX, int offsetY) {
    ...
    MapVinaMap mapvinaMap = this.mapvinaMap.get();
    View view = this.view.get();
    if (view != null && mapvinaMap != null) {
      ...
      coordinates = mapvinaMap.getProjection().toScreenLocation(position);
      ...
      if (view instanceof BubbleLayout) {
        ...
        float marginHorizontal = resources.getDimension(R.dimen.mapvina_infowindow_margin);
        float tipViewOffset = resources.getDimension(R.dimen.mapvina_infowindow_tipview_width) / 2;
  ...
  InfoWindow close() {
    ...
    if (isVisible && mapvinaMap != null) {
      ...
      MapVinaMap.OnInfoWindowCloseListener listener = mapvinaMap.getOnInfoWindowCloseListener();
      if (listener != null) {
   ...
   */
  void adaptDefaultMarker(@NonNull Marker overlayItem, MapVinaMap mapvinaMap, @NonNull MapView mapView) {
    View view = this.view.get();
    if (view == null) {
  ...
  public void update() {
    ...
    Marker marker = boundMarker.get();
    View view = this.view.get();
    if (mapvinaMap != null && marker != null && view != null) {
      coordinates = mapvinaMap.getProjection().toScreenLocation(marker.getPosition());
