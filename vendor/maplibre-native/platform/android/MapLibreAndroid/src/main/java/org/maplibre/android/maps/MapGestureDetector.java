...
import static com.mapvina.android.constants.MapVinaConstants.MAXIMUM_ANGULAR_VELOCITY;
import static com.mapvina.android.constants.MapVinaConstants.MAX_ABSOLUTE_SCALE_VELOCITY_CHANGE;
import static com.mapvina.android.constants.MapVinaConstants.QUICK_ZOOM_MAX_ZOOM_CHANGE;
import static com.mapvina.android.constants.MapVinaConstants.ROTATE_VELOCITY_RATIO_THRESHOLD;
import static com.mapvina.android.constants.MapVinaConstants.SCALE_VELOCITY_ANIMATION_DURATION_MULTIPLIER;
import static com.mapvina.android.constants.MapVinaConstants.SCALE_VELOCITY_RATIO_THRESHOLD;
import static com.mapvina.android.constants.MapVinaConstants.ZOOM_RATE;
import static com.mapvina.android.maps.MapVinaMap.OnCameraMoveStartedListener.REASON_API_GESTURE;
import static com.mapvina.android.utils.MathUtils.normalize;
...
final class MapGestureDetector {
  ...
  // new map touch API
  private final CopyOnWriteArrayList<MapVinaMap.OnMapClickListener> onMapClickListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<MapVinaMap.OnMapLongClickListener> onMapLongClickListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<MapVinaMap.OnFlingListener> onFlingListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<MapVinaMap.OnMoveListener> onMoveListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<MapVinaMap.OnRotateListener> onRotateListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<MapVinaMap.OnScaleListener> onScaleListenerList
    = new CopyOnWriteArrayList<>();

  private final CopyOnWriteArrayList<MapVinaMap.OnShoveListener> onShoveListenerList
    = new CopyOnWriteArrayList<>();
   ...
   * {@link MapVinaConstants#SCHEDULED_ANIMATION_TIMEOUT}
   */
  @NonNull
    ...
    if (attachDefaultListeners) {
      StandardGestureListener standardGestureListener = new StandardGestureListener(
        context.getResources().getDimension(
          com.mapvina.android.gestures.R.dimen.mapbox_defaultScaleSpanSinceStartThreshold));
      ...
      ScaleGestureListener scaleGestureListener = new ScaleGestureListener(
        context.getResources().getDimension(R.dimen.mapvina_density_constant),
        context.getResources().getDimension(R.dimen.mapvina_minimum_scale_speed),
        context.getResources().getDimension(R.dimen.mapvina_minimum_angled_scale_speed),
        context.getResources().getDimension(R.dimen.mapvina_minimum_scale_velocity)
      );
      RotateGestureListener rotateGestureListener = new RotateGestureListener(
        context.getResources().getDimension(R.dimen.mapvina_minimum_scale_span_when_rotating),
        context.getResources().getDimension(R.dimen.mapvina_density_constant),
        context.getResources().getDimension(R.dimen.mapvina_angular_velocity_multiplier),
        context.getResources().getDimension(R.dimen.mapvina_minimum_angular_velocity),
        context.getResources().getDimension(
          com.mapvina.android.gestures.R.dimen.mapbox_defaultScaleSpanSinceStartThreshold));
  ...
  private void initializeGesturesManager(@NonNull AndroidGesturesManager androidGesturesManager,
    ...
    androidGesturesManager.getMoveGestureDetector().setMoveThresholdResource(R.dimen.mapvina_minimum_move_threshold);
  ...
  private void scheduleAnimator(Animator animator) {
    ...
    animationsTimeoutHandler.removeCallbacksAndMessages(null);
    animationsTimeoutHandler.postDelayed(cancelAnimatorsRunnable, MapVinaConstants.SCHEDULED_ANIMATION_TIMEOUT);
  }
  ...
  private final class StandardGestureListener extends StandardGestureDetector.SimpleStandardOnGestureListener {
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
      ...
      if (!uiSettings.isHorizontalScrollGesturesEnabled()) {
        ...
        if (angle > MapVinaConstants.ANGLE_THRESHOLD_IGNORE_VERTICAL_FLING) {
          return false;
        }
        offsetX = 0.0;
      }
  ...
  private final class ShoveGestureListener extends ShoveGestureDetector.SimpleOnShoveGestureListener {
    ...
    public boolean onShove(@NonNull ShoveGestureDetector detector,
      ...
      pitch -= MapVinaConstants.SHOVE_PIXEL_CHANGE_FACTOR * deltaPixelsSinceLast;
      pitch = MathUtils.clamp(pitch, MapVinaConstants.MINIMUM_TILT, MapVinaConstants.MAXIMUM_TILT);
  ...
  private void zoomAnimated(boolean zoomIn, @NonNull PointF zoomFocalPoint, boolean runImmediately) {
    ...
    scaleAnimator = createScaleAnimator(
      ...
      zoomIn ? 1 : -1,
      zoomFocalPoint,
      MapVinaConstants.ANIMATION_DURATION);
    if (runImmediately) {
      scaleAnimator.start();
    } else {
      scheduleAnimator(scaleAnimator);
  ...
  void notifyOnMapClickListeners(@NonNull PointF tapPoint) {
    for (MapVinaMap.OnMapClickListener listener : onMapClickListenerList) {
  ...
  void notifyOnMapLongClickListeners(@NonNull PointF longClickPoint) {
    for (MapVinaMap.OnMapLongClickListener listener : onMapLongClickListenerList) {
  ...
  void notifyOnFlingListeners() {
    for (MapVinaMap.OnFlingListener listener : onFlingListenerList) {
      listener.onFling();
    }
  }

  void notifyOnMoveBeginListeners(@NonNull MoveGestureDetector detector) {
    for (MapVinaMap.OnMoveListener listener : onMoveListenerList) {
  ...
  void notifyOnRotateBeginListeners(@NonNull RotateGestureDetector detector) {
    for (MapVinaMap.OnRotateListener listener : onRotateListenerList) {
  ...
  void notifyOnScaleBeginListeners(@NonNull StandardScaleGestureDetector detector) {
    for (MapVinaMap.OnScaleListener listener : onScaleListenerList) {
  ...
  void notifyOnShoveBeginListeners(@NonNull ShoveGestureDetector detector) {
    for (MapVinaMap.OnShoveListener listener : onShoveListenerList) {
  ...
  void addOnMapClickListener(MapVinaMap.OnMapClickListener onMapClickListener) {
    onMapClickListenerList.add(onMapClickListener);
  }

  void removeOnMapClickListener(MapVinaMap.OnMapClickListener onMapClickListener) {
    onMapClickListenerList.remove(onMapClickListener);
  }

  void addOnMapLongClickListener(MapVinaMap.OnMapLongClickListener onMapLongClickListener) {
    onMapLongClickListenerList.add(onMapLongClickListener);
  }

  void removeOnMapLongClickListener(MapVinaMap.OnMapLongClickListener onMapLongClickListener) {
    onMapLongClickListenerList.remove(onMapLongClickListener);
  }

  void addOnFlingListener(MapVinaMap.OnFlingListener onFlingListener) {
    onFlingListenerList.add(onFlingListener);
  }

  void removeOnFlingListener(MapVinaMap.OnFlingListener onFlingListener) {
    onFlingListenerList.remove(onFlingListener);
  }

  void addOnMoveListener(MapVinaMap.OnMoveListener listener) {
    onMoveListenerList.add(listener);
  }

  void removeOnMoveListener(MapVinaMap.OnMoveListener listener) {
    onMoveListenerList.remove(listener);
  }

  void addOnRotateListener(MapVinaMap.OnRotateListener listener) {
    onRotateListenerList.add(listener);
  }

  void removeOnRotateListener(MapVinaMap.OnRotateListener listener) {
    onRotateListenerList.remove(listener);
  }

  void addOnScaleListener(MapVinaMap.OnScaleListener listener) {
    onScaleListenerList.add(listener);
  }

  void removeOnScaleListener(MapVinaMap.OnScaleListener listener) {
    onScaleListenerList.remove(listener);
  }

  void addShoveListener(MapVinaMap.OnShoveListener listener) {
    onShoveListenerList.add(listener);
  }

  void removeShoveListener(MapVinaMap.OnShoveListener listener) {
