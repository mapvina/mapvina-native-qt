...
import static com.mapvina.android.location.engine.Utils.isBetterLocation;

/**
 * MapVina replacement for Google Play Services Fused Location Client
 * <p>
 * Note: fusion will not work in background mode.
 */
public class MapVinaFusedLocationEngineImpl extends AndroidLocationEngineImpl {
  private static final String TAG = "MapVinaLocationEngine";

  public MapVinaFusedLocationEngineImpl(@NonNull Context context) {
    super(context);
  }
  ...
  public LocationListener createListener(LocationEngineCallback<LocationEngineResult> callback) {
    return new MapVinaLocationEngineCallbackTransport(callback);
  }
  ...
  private static final class MapVinaLocationEngineCallbackTransport implements LocationListener {
    private final LocationEngineCallback<LocationEngineResult> callback;
    private Location currentBestLocation;

    MapVinaLocationEngineCallbackTransport(LocationEngineCallback<LocationEngineResult> callback) {
