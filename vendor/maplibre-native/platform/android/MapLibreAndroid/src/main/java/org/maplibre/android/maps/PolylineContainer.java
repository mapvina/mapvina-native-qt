...
class PolylineContainer implements Polylines {
  ...
  @Override
  public Polyline addBy(@NonNull PolylineOptions polylineOptions, @NonNull MapVinaMap mapvinaMap) {
    Polyline polyline = polylineOptions.getPolyline();
    long id = nativeMap != null ? nativeMap.addPolyline(polyline) : 0;
    polyline.setMapVinaMap(mapvinaMap);
    polyline.setId(id);
  ...
  @NonNull
  @Override
  public List<Polyline> addBy(@NonNull List<PolylineOptions> polylineOptionsList, @NonNull MapVinaMap mapvinaMap) {
    ...
    if (nativeMap != null && count > 0) {
      ...
      for (int i = 0; i < ids.length; i++) {
        ...
        polylineCreated.setMapVinaMap(mapvinaMap);
