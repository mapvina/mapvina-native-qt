...
class IconManager {
  ...
  private Icon loadDefaultIconForMarker(Marker marker) {
    Icon icon = IconFactory.getInstance(MapVina.getApplicationContext()).defaultMarker();
  ...
  void ensureIconLoaded(@NonNull Marker marker, @NonNull MapVinaMap mapvinaMap) {
    Icon icon = marker.getIcon();
    if (icon == null) {
      icon = loadDefaultIconForMarker(marker);
    }
    addIcon(icon);
    setTopOffsetPixels(marker, mapvinaMap, icon);
  }

  private void setTopOffsetPixels(Marker marker, @NonNull MapVinaMap mapvinaMap, @NonNull Icon icon) {
    // this seems to be a costly operation according to the profiler so I'm trying to save some calls
    Marker previousMarker = marker.getId() != -1 ? (Marker) mapvinaMap.getAnnotation(marker.getId()) : null;
