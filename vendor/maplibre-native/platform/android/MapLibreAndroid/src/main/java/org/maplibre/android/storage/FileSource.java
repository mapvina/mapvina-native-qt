...
import com.mapvina.android.MapStrictMode;
...
import com.mapvina.android.utils.FileUtils;
...
public class FileSource {
  ...
  private static String getCachePath(@NonNull Context context) {
    SharedPreferences preferences = context.getSharedPreferences(
      MapVinaConstants.MAPVINA_SHARED_PREFERENCES, Context.MODE_PRIVATE);
    ...
    if (!isPathWritable(cachePath)) {
      ...
      SharedPreferences.Editor editor =
        context.getSharedPreferences(MapVinaConstants.MAPVINA_SHARED_PREFERENCES, Context.MODE_PRIVATE).edit();
  ...
  private static boolean isExternalStorageConfiguration(@NonNull Context context) {
    // Default value
    boolean isExternalStorageConfiguration = MapVinaConstants.DEFAULT_SET_STORAGE_EXTERNAL;
    ...
    try {
      ...
      if (appInfo.metaData != null) {
        isExternalStorageConfiguration = appInfo.metaData.getBoolean(
          MapVinaConstants.KEY_META_DATA_SET_STORAGE_EXTERNAL,
          MapVinaConstants.DEFAULT_SET_STORAGE_EXTERNAL
        );
      }
  ...
  public static void setResourcesCachePath(@NonNull final String path,
                                           @NonNull final ResourcesCachePathChangeCallback callback) {
    final Context applicationContext = MapVina.getApplicationContext();
      ...
      new FileUtils.CheckFileWritePermissionTask(new FileUtils.OnCheckFileWritePermissionListener() {
        ...
        public void onWritePermissionGranted() {
          final SharedPreferences.Editor editor =
            applicationContext.getSharedPreferences(MapVinaConstants.MAPVINA_SHARED_PREFERENCES,
  ...
  private FileSource(String cachePath) {
    TileServerOptions options = MapVina.getTileServerOptions();
    initialize(MapVina.getApiKey(), cachePath, options);
  }
