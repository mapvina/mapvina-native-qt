...
object CameraUpdateFactory {
    ...
    class CameraPositionUpdate(val bearing: Double, val target: LatLng?, val tilt: Double, val zoom: Double, val padding: DoubleArray?) : CameraUpdate {
        ...
        override fun getCameraPosition(mapvinaMap: MapVinaMap): CameraPosition {
            if (target == null) {
                val previousPosition = mapvinaMap.cameraPosition
    ...
    internal class CameraBoundsUpdate(val bounds: LatLngBounds, private val bearing: Double?, private val tilt: Double?, val padding: IntArray) : CameraUpdate {
        ...
        override fun getCameraPosition(mapvinaMap: MapVinaMap): CameraPosition? {
            return if (bearing == null && tilt == null) {
                ...
                mapvinaMap.getCameraForLatLngBounds(bounds, padding)
            } else {
                // use provided tilt and bearing
                assert(bearing != null)
                assert(tilt != null)
                mapvinaMap.getCameraForLatLngBounds(bounds, padding, bearing!!, tilt!!)
            }
        }
    ...
    class ZoomUpdate : CameraUpdate {
        ...
        override fun getCameraPosition(mapvinaMap: MapVinaMap): CameraPosition {
            ...
            return if (type != ZOOM_TO_POINT) {
                ...
                CameraPosition.Builder(cameraPosition).zoom(transformZoom(cameraPosition.zoom)).target(mapvinaMap.projection.fromScreenLocation(PointF(x, y))).build()
            }
        }
