...
final class LocationLayerController {
  ...
  LocationLayerController(MapVinaMap mapvinaMap, Style style,
  ...
  private void styleScaling(@NonNull LocationComponentOptions options) {
    Expression scaleExpression = interpolate(linear(), zoom(),
      stop(mapvinaMap.getMinZoomLevel(), options.minZoomIconScale()),
      stop(mapvinaMap.getMaxZoomLevel(), options.maxZoomIconScale())
    );
  ...
  boolean onMapClick(@NonNull LatLng point) {
    PointF screenLoc = mapvinaMap.getProjection().toScreenLocation(point);
    List<Feature> features = mapvinaMap.queryRenderedFeatures(screenLoc,
      BACKGROUND_LAYER,
      FOREGROUND_LAYER,
      BEARING_LAYER
    );
  ...
  private final MapVinaAnimator.AnimationsValueChangeListener<Float> accuracyValueListener =
    new MapVinaAnimator.AnimationsValueChangeListener<Float>() {
      @Override
      public void onNewAnimationValue(Float value) {
   ...
   */
  private final MapVinaAnimator.AnimationsValueChangeListener<Float> pulsingCircleRadiusListener =
    new MapVinaAnimator.AnimationsValueChangeListener<Float>() {
      @Override
  ...
  Set<AnimatorListenerHolder> getAnimationListeners() {
    ...
    holders.add(new AnimatorListenerHolder(MapVinaAnimator.ANIMATOR_LAYER_LATLNG, latLngValueListener));

    if (renderMode == RenderMode.GPS) {
      holders.add(new AnimatorListenerHolder(MapVinaAnimator.ANIMATOR_LAYER_GPS_BEARING, gpsBearingValueListener));
    } else if (renderMode == RenderMode.COMPASS) {
      holders.add(
        new AnimatorListenerHolder(MapVinaAnimator.ANIMATOR_LAYER_COMPASS_BEARING, compassBearingValueListener));
    }

    if (renderMode == RenderMode.COMPASS || renderMode == RenderMode.NORMAL) {
      holders.add(new AnimatorListenerHolder(MapVinaAnimator.ANIMATOR_LAYER_ACCURACY, accuracyValueListener));
    }

    if (options.pulseEnabled()) {
      holders.add(new AnimatorListenerHolder(MapVinaAnimator.ANIMATOR_PULSING_CIRCLE,
