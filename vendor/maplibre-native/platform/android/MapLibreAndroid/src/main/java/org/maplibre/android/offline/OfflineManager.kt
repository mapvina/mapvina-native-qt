...
import com.mapvina.android.geometry.LatLngBounds.Companion.world
...
class OfflineManager private constructor(context: Context) {
     ...
     *
     * Forces MapVina Native to revalidate resources stored in the ambient
    ...
    fun createOfflineRegion(definition: OfflineRegionDefinition, metadata: ByteArray, callback: CreateOfflineRegionCallback) {
        if (!isValidOfflineRegionDefinition(definition)) {
            callback.onError(String.format(context.getString(R.string.mapvina_offline_error_region_definition_invalid), definition.bounds))
            return
        }
