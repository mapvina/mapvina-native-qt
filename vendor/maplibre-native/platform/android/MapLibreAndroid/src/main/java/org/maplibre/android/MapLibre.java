...
/**
 * The entry point to initialize the MapVina Android SDK.
 * <p>
...
public final class MapVina {
  ...
  private static final String TAG = "Mbgl-MapVina";
  private static ModuleProvider moduleProvider;
  private static MapVina INSTANCE;
   ...
   * @return the single instance of MapVina
   */
  @UiThread
  @NonNull
  public static synchronized MapVina getInstance(@NonNull Context context) {
    ThreadUtils.init(context);
    ThreadUtils.checkThread(TAG);
    if (INSTANCE == null) {
      Context appContext = context.getApplicationContext();
      FileSource.initializeFileDirsPaths(appContext);
      INSTANCE = new MapVina(appContext, null);
      ConnectivityReceiver.instance(appContext);
    }

    TileServerOptions tileServerOptions = TileServerOptions.get(WellKnownTileServer.MapVina);
  ...
  @UiThread
  @NonNull
  public static synchronized MapVina getInstance(@NonNull Context context, @Nullable String apiKey,
    ...
    if (INSTANCE == null) {
      ...
      Context appContext = context.getApplicationContext();
      FileSource.initializeFileDirsPaths(appContext);
      INSTANCE = new MapVina(appContext, apiKey);
      ConnectivityReceiver.instance(appContext);
    } else {
  ...
  MapVina(@NonNull Context context, @Nullable String apiKey) {
    this.context = context;
    this.apiKey = apiKey;
  }

  MapVina(@NonNull Context context, @Nullable String apiKey, @NonNull TileServerOptions options) {
    this.context = context;
    this.apiKey = apiKey;
  ...
  /**
   * Runtime validation of MapVina creation.
   */
  private static void validateMapVina() {
    if (INSTANCE == null) {
  ...
  /**
   * Runtime validation of MapVina access token
   *
  ...
  static boolean isApiKeyValid(@Nullable String apiKey) {
    ...
    apiKey = apiKey.trim().toLowerCase(MapVinaConstants.MAPVINA_LOCALE);
  ...
  /**
   * Internal use. Check if the {@link MapVina#INSTANCE} is present.
   */
