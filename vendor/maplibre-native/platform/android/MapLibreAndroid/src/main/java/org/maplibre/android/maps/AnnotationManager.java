 ...
 * annotations that occur on {@link MapVinaMap} are forwarded to this class.
 * <p>
...
class AnnotationManager {
  ...
  @Nullable
  private MapVinaMap.OnMarkerClickListener onMarkerClickListener;
  @Nullable
  private MapVinaMap.OnPolygonClickListener onPolygonClickListener;
  @Nullable
  private MapVinaMap.OnPolylineClickListener onPolylineClickListener;
  ...
  // TODO refactor MapVinaMap out for Projection and Transform
  // Requires removing MapVinaMap from Annotations by using Peer model from #6912
  @NonNull
  AnnotationManager bind(MapVinaMap mapvinaMap) {
  ...
  Marker addMarker(@NonNull BaseMarkerOptions markerOptions, @NonNull MapVinaMap mapvinaMap) {
    return markers.addBy(markerOptions, mapvinaMap);
  }

  List<Marker> addMarkers(@NonNull List<? extends BaseMarkerOptions> markerOptionsList,
                          @NonNull MapVinaMap mapvinaMap) {
    return markers.addBy(markerOptionsList, mapvinaMap);
  }

  void updateMarker(@NonNull Marker updatedMarker, @NonNull MapVinaMap mapvinaMap) {
    if (!isAddedToMap(updatedMarker)) {
      logNonAdded(updatedMarker);
      return;
    }
    markers.update(updatedMarker, mapvinaMap);
  }
  ...
  Polygon addPolygon(@NonNull PolygonOptions polygonOptions, @NonNull MapVinaMap mapvinaMap) {
    return polygons.addBy(polygonOptions, mapvinaMap);
  }

  List<Polygon> addPolygons(@NonNull List<PolygonOptions> polygonOptionsList, @NonNull MapVinaMap mapvinaMap) {
    return polygons.addBy(polygonOptionsList, mapvinaMap);
  }
  ...
  Polyline addPolyline(@NonNull PolylineOptions polylineOptions, @NonNull MapVinaMap mapvinaMap) {
    return polylines.addBy(polylineOptions, mapvinaMap);
  }

  List<Polyline> addPolylines(@NonNull List<PolylineOptions> polylineOptionsList, @NonNull MapVinaMap mapvinaMap) {
    return polylines.addBy(polylineOptionsList, mapvinaMap);
  }
  ...
  void setOnMarkerClickListener(@Nullable MapVinaMap.OnMarkerClickListener listener) {
    onMarkerClickListener = listener;
  }

  void setOnPolygonClickListener(@Nullable MapVinaMap.OnPolygonClickListener listener) {
    onPolygonClickListener = listener;
  }

  void setOnPolylineClickListener(@Nullable MapVinaMap.OnPolylineClickListener listener) {
  ...
  void selectMarker(@NonNull Marker marker) {
    ...
    if (infoWindowManager.isInfoWindowValidForMarker(marker) || infoWindowManager.getInfoWindowAdapter() != null) {
      infoWindowManager.add(marker.showInfoWindow(mapvinaMap, mapView));
    }
  ...
  void adjustTopOffsetPixels(@NonNull MapVinaMap mapvinaMap) {
    int count = annotationsArray.size();
    for (int i = 0; i < count; i++) {
    ...
    for (Marker marker : selectedMarkers) {
      if (marker.isInfoWindowShown()) {
        marker.hideInfoWindow();
        marker.showInfoWindow(mapvinaMap, mapView);
      }
    }
  }
  ...
  boolean onTap(@NonNull PointF tapPoint) {
    ...
    long markerId = new MarkerHitResolver(mapvinaMap).execute(markerHit);
  ...
  private ShapeAnnotationHit getShapeAnnotationHitFromTap(PointF tapPoint) {
    float touchTargetSide = MapVina.getApplicationContext().getResources().getDimension(R.dimen.mapvina_eight_dp);
  ...
  private static class MarkerHitResolver {
    ...
    MarkerHitResolver(@NonNull MapVinaMap mapvinaMap) {
      this.projection = mapvinaMap.getProjection();
      this.minimalTouchSize = (int) (32 * MapVina.getApplicationContext().getResources().getDisplayMetrics().density);
    }
