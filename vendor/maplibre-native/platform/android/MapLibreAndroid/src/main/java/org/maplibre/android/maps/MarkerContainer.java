...
class MarkerContainer implements Markers {
  ...
  @Override
  public Marker addBy(@NonNull BaseMarkerOptions markerOptions, @NonNull MapVinaMap mapvinaMap) {
    Marker marker = prepareMarker(markerOptions);
    long id = nativeMapView != null ? nativeMapView.addMarker(marker) : 0;
    marker.setMapVinaMap(mapvinaMap);
    marker.setId(id);
  ...
  @NonNull
  @Override
  public List<Marker> addBy(@NonNull List<? extends BaseMarkerOptions> markerOptionsList, @NonNull MapVinaMap
      mapvinaMap) {
    ...
    if (nativeMapView != null && count > 0) {
      ...
      if (markers.size() > 0) {
        ...
        for (int i = 0; i < ids.length; i++) {
          ...
          createdMarker.setMapVinaMap(mapvinaMap);
  ...
  @Override
  public void update(@NonNull Marker updatedMarker, @NonNull MapVinaMap mapvinaMap) {
    ensureIconLoaded(updatedMarker, mapvinaMap);
  ...
  public List<Marker> obtainAllIn(@NonNull RectF rectangle) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (annotation instanceof com.mapvina.android.annotations.Marker && idsList.contains(annotation.getId())) {
        annotations.add((com.mapvina.android.annotations.Marker) annotation);
      }
    }
  ...
  private void ensureIconLoaded(Marker marker, @NonNull MapVinaMap mapvinaMap) {
    iconManager.ensureIconLoaded(marker, mapvinaMap);
  }
