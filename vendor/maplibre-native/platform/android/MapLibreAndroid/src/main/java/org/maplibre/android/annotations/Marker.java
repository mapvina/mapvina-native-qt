 ...
 * {@link LatLng} and using {@link MapVinaMap#addMarker(MarkerOptions)}. The marker icon will be
...
public class Marker extends Annotation {
  ...
  private void refreshInfoWindowContent() {
    if (isInfoWindowShown() && mapView != null && mapvinaMap != null && mapvinaMap.getInfoWindowAdapter() == null) {
      InfoWindow infoWindow = getInfoWindow(mapView);
      if (mapView.getContext() != null) {
        infoWindow.adaptDefaultMarker(this, mapvinaMap, mapView);
      }
      MapVinaMap map = getMapVinaMap();
      if (map != null) {
        map.updateMarker(this);
  ...
  /**
   * Do not use this method, used internally by the SDK. Use {@link MapVinaMap#selectMarker(Marker)}
   * if you want to programmatically display the markers info window.
   *
   * @param mapvinaMap The hosting MapVinaMap.
   * @param mapView   The hosting map view.
   * @return The info window that was shown.
   */
  @Nullable
  public InfoWindow showInfoWindow(@NonNull MapVinaMap mapvinaMap, @NonNull MapView mapView) {
    setMapVinaMap(mapvinaMap);
    setMapView(mapView);
    MapVinaMap.InfoWindowAdapter infoWindowAdapter = getMapVinaMap().getInfoWindowAdapter();
    if (infoWindowAdapter != null) {
      // end developer is using a custom InfoWindowAdapter
      View content = infoWindowAdapter.getInfoWindow(this);
      if (content != null) {
        infoWindow = new InfoWindow(content, mapvinaMap);
  ...
  private InfoWindow getInfoWindow(@NonNull MapView mapView) {
    if (infoWindow == null && mapView.getContext() != null) {
      infoWindow = new InfoWindow(mapView, R.layout.mapvina_infowindow_content, getMapVinaMap());
    }
    return infoWindow;
  }
