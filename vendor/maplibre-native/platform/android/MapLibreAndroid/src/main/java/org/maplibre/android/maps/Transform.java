...
import static com.mapvina.android.maps.MapVinaMap.OnCameraMoveStartedListener;
...
public class Transform implements MapView.OnCameraDidChangeListener {
  ...
  @Nullable
  private MapVinaMap.CancelableCallback cameraCancelableCallback;
  ...
  void initialise(@NonNull MapVinaMap mapvinaMap, @NonNull MapVinaMapOptions options) {
    CameraPosition position = options.getCamera();
    if (position != null && !position.equals(CameraPosition.DEFAULT)) {
      moveCamera(mapvinaMap, CameraUpdateFactory.newCameraPosition(position), null);
    }
  ...
  public void onCameraDidChange(boolean animated) {
    if (animated) {
      ...
      if (cameraCancelableCallback != null) {
        final MapVinaMap.CancelableCallback callback = cameraCancelableCallback;
   ...
   */
  @UiThread
  public void moveCamera(@NonNull MapVinaMap mapvinaMap, CameraUpdate update,
                               @Nullable final MapVinaMap.CancelableCallback callback) {
    CameraPosition cameraPosition = update.getCameraPosition(mapvinaMap);
  ...
  @UiThread
  void easeCamera(@NonNull MapVinaMap mapvinaMap, CameraUpdate update, int durationMs,
   ...
   */
  @UiThread
  public void animateCamera(@NonNull MapVinaMap mapvinaMap, CameraUpdate update, int durationMs,
                                  @Nullable final MapVinaMap.CancelableCallback callback) {
    CameraPosition cameraPosition = update.getCameraPosition(mapvinaMap);
    if (isValidCameraPosition(cameraPosition)) {
      cancelTransitions();
  ...
  void setMinZoom(double minZoom) {
    if ((minZoom < MapVinaConstants.MINIMUM_ZOOM) || (minZoom > MapVinaConstants.MAXIMUM_ZOOM)) {
  ...
  void setMaxZoom(double maxZoom) {
    if ((maxZoom < MapVinaConstants.MINIMUM_ZOOM) || (maxZoom > MapVinaConstants.MAXIMUM_ZOOM)) {
  ...
  void setMinPitch(double minPitch) {
    if ((minPitch < MapVinaConstants.MINIMUM_PITCH) || (minPitch > MapVinaConstants.MAXIMUM_PITCH)) {
  ...
  void setMaxPitch(double maxPitch) {
    if ((maxPitch < MapVinaConstants.MINIMUM_PITCH) || (maxPitch > MapVinaConstants.MAXIMUM_PITCH)) {
