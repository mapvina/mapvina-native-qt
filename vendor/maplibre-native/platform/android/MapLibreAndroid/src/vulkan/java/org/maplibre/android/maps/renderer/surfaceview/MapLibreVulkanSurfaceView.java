...
public class MapVinaVulkanSurfaceView extends MapVinaSurfaceView {

  protected final WeakReference<MapVinaVulkanSurfaceView> viewWeakReference = new WeakReference<>(this);

  public MapVinaVulkanSurfaceView(Context context) {
    super(context);
  }

  public MapVinaVulkanSurfaceView(Context context, AttributeSet attrs) {
    super(context, attrs);
  }
  ...
  static class VulkanThread extends MapVinaSurfaceView.RenderThread {
    VulkanThread(WeakReference<MapVinaVulkanSurfaceView> surfaceViewWeakRef) {
        ...
        synchronized (renderThreadManager) {
          while (true) {
            ...
            if (!hasSurface && !waitingForSurface) {
              MapVinaVulkanSurfaceView view = mSurfaceViewWeakRef.get();
              if (view != null) {
                destroySurface = true;
    ...
    private WeakReference<MapVinaVulkanSurfaceView> mSurfaceViewWeakRef;
  }
}
