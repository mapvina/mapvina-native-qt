...
fun Project.nativeBuild(nativeTargets: List<String>) =
    this.extensions.getByType<BaseExtension>().run {
        ...
        //
        // When invoking from the command line or to override the device default, set `-Pmapvina.abis=...` to
        ...
        var abi = "all"
        if (!project.hasProperty("android.injected.invoked.from.ide") && project.hasProperty("mapvina.abis")) {
            abi = project.property("mapvina.abis") as String
        }

        if (abi != "none") {
            externalNativeBuild {
                cmake {
                    path = file("../MapVinaAndroid/src/cpp/CMakeLists.txt")
