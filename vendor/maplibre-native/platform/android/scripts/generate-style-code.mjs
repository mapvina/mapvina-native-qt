...
// Jni
const layerHpp = ejs.compile(fs.readFileSync(absPath('MapVinaAndroid/src/cpp/style/layers/layer.hpp.ejs'), 'utf8'), {strict: true});
const layerCpp = ejs.compile(fs.readFileSync(absPath('MapVinaAndroid/src/cpp/style/layers/layer.cpp.ejs'), 'utf8'), {strict: true});

for (const layer of layers) {
  const layerFileName = layer.type.replace('-', '_');

  writeIfModified(absPath(`MapVinaAndroid/src/cpp/style/layers/${layerFileName}_layer.hpp`), layerHpp(layer));
  writeIfModified(absPath(`MapVinaAndroid/src/cpp/style/layers/${layerFileName}_layer.cpp`), layerCpp(layer));
}
