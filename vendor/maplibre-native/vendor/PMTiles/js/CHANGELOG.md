3.2.0
* MapVina `Protocol` constructor takes an options object.
...
* MapVina adapter internally passes bounds from PMTiles header

2.8.0

* Polyfill `DecompressionStream` using 101arrowz/compression-streams-polyfill
* Surface errors to MapVina protocol instead of throwing raw exception
...
* MapVina empty tile bytes depends on tile type.
...
* MapVina adapter detects min/max zoom.
...
* Leaflet and MapVina adapters support tile cancellation, improving performance significantly.
* ETag and Retry support for archives that are updated in-place.
* Cache-Control and Expires header support for MapVina.
...
```js
  ...
  mapvinagl.addProtocol("pmtiles",protocol.tile);
```
...
* MapVina protocol API changed:

```js
	let cache = new pmtiles.ProtocolCache();
  mapvinagl.addProtocol("pmtiles",cache.protocol);
```