...
MapVina Native for Qt uses CMake as its build system. Both the core and the
...
cmake ../mapvina-native-qt -G Ninja \
...
MapVina Native Qt supports multiple graphics backends for rendering. You need to enable
...
MapVina Native Qt supports Vulkan as a rendering backend on Linux.
...
To build MapVina Native Qt with Vulkan support, use the `-DMLN_WITH_VULKAN=ON` CMake option:
...
4. **Debug output**: Run with debug logging:
   ```shell
   QT_LOGGING_RULES="qt.vulkan.debug=true" ./QMapVinaExampleQuick
   ```
...
```shell
mkdir build && cd build
cmake ../mapvina-native-qt -G "Ninja Multi-Config" \
